...
export async function initHubtelPayment({ phone, name, amount, reference, description, returnUrl, cancelUrl }) {
  console.log('[Hubtel] checkout using merchantId:', process.env.HUBTEL_MERCHANT_ID || 'MISSING');
  console.log('[Hubtel] checkout clientId set:', !!process.env.HUBTEL_CLIENT_ID);
  console.log('[Hubtel] checkout clientSecret set:', !!process.env.HUBTEL_CLIENT_SECRET);
  let res, text;
  try {
    ...
    throw new Error(`Hubtel fetch failed: ${fetchErr.message}`);
  }
  let data;
  try { data = JSON.parse(text); }
  catch (_) { throw new Error(`Hubtel returned non-JSON: ${text.slice(0, 120)}`); }

  console.log('[Hubtel] checkout status:', res.status);
  console.log('[Hubtel] checkout raw:', text.slice(0, 300));
...
export async function initHubtelDirectDebit({ phone, amount, reference, description, network }) {
  ...
  console.log('[Hubtel] direct debit status:', res.status);
  console.log('[Hubtel] direct debit raw:', text.slice(0, 300));
